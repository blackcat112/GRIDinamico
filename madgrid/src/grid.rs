use anyhow::Result;
use geo::{algorithm::centroid::Centroid, Contains, Point, Polygon, BoundingRect};
...
#[derive(Clone)]
pub struct HexCell { pub id: u32, pub poly: Polygon, pub centroid: (f32,f32), pub aabb: AABB<[f32;2]> }

#[derive(Clone)]
pub struct GridIndex { pub cells: Vec<HexCell>, tree: RTree<GridItem> }

#[derive(Clone, Copy)]
pub struct GridItem { pub id: u32, pub aabb: AABB<[f32;2]> }
...
impl GridIndex {
    pub fn from_geojson(path: &str) -> Result<Self> {
        ...
        let fc = match gj { GeoJson::FeatureCollection(fc) => fc, _ => anyhow::bail!("GeoJSON debe ser FeatureCollection") };
        let mut cells: Vec<HexCell> = Vec::new();
        let mut items: Vec<GridItem> = Vec::new();
        for (idx, feat) in fc.features.into_iter().enumerate() {
            if let Some(geom) = feat.geometry { if let Some((poly, aabb)) = polygon_from_geometry(&geom) {
                let cent = poly.centroid().map(|p| (p.y() as f32, p.x() as f32)).unwrap_or((0.0,0.0));
                let id = idx as u32;
                let aabb = AABB::from_corners([aabb.min().x as f32, aabb.min().y as f32], [aabb.max().x as f32, aabb.max().y as f32]);
...
fn polygon_from_geometry(g: &Geometry) -> Option<(Polygon, geo::Rect)> {
    match &g.value {
        Value::Polygon(coords) => {
            ...
            let rect = poly.bounding_rect()?; Some((poly, rect))
        }
        Value::MultiPolygon(multi) => {
            // Usamos sólo el primer polígono (como hacías en Node‑RED)
            if let Some(first) = multi.get(0) { let exterior: Vec<_> = first[0].iter().map(|c| (c[0], c[1])).collect();
                let poly: Polygon = Polygon::new(exterior.into(), vec![]);
                let rect = poly.bounding_rect()?; Some((poly, rect)) } else { None }
        }
        _ => None,
    }
}
...
pub fn recompute(
    ...
    // Pre‑compute parking R‑Tree? Para pocos miles no hace falta; usamos distancia directa
    ...
    for cell in &grid.cells {
        ...
        if n > 0.0 {
            ...
            nivel_avg /= n.max(1.0);
            ocup_avg /= n.max(1.0);
            if !vels.is_empty() { vels.sort_by(|a,b| a.partial_cmp(b).unwrap()); let m = vels.len()/2; vel_med = if vels.len()%2==1 { vels[m] } else { (vels[m-1]+vels[m])/2.0 }; }
        }
        ...
        else if (tr_c.len() as u8) < cfg.min_sens_ok { let f = ((tr_c.len() as i32 - cfg.min_sens_any as i32) as f32) / ((cfg.min_sens_ok - cfg.min_sens_any) as f32); conf = clamp(f, 0.4, 1.0); }
        let traffic_contrib = conf*(cfg.w_carga*carga01 + cfg.w_nivel*nivel01 + cfg.w_velinv*velinv + cfg.w_ocup*occ01);
        ...
        let delay = if blocked { None } else { Some(clamp(1.0 + traffic_contrib + pen, cfg.delay_min, cfg.delay_max)) };

        // Parking cercano (conteo + mínima distancia)
        let (clat, clon) = cell.centroid;
        let mut count = 0usize; let mut dmin = f32::INFINITY;
        for z in cargas { let d = haversine_m(clat, clon, z.lat, z.lon); if d <= cfg.park_radius_m { count += 1; if d < dmin { dmin = d; } } }
        ...
        // Construir feature sólo si hay delay > 1+eps
        if let Some(df) = delay { if df > 1.0 + cfg.show_eps {
        ...
        outs.push(CellOut {
            ...
            incidencias: incs_c.len(),
            carga_near_count: count,
            carga_min_dist_m: (park_min*100.0).round()/100.0, // guardamos park_min como métrica
...
fn color_from_norm(x: f32) -> String {
    const RAMP: [&str; 11] = [
        "#e9f7ef","#d4f2e3","#bfeacc","#a9e3b6","#fff3b0","#ffe08a","#ffc266","#ff9f58","#ff7a55","#f5544f","#d73a49"
    ];
